...
impl Codegen {
    pub unsafe fn compile_term(&mut self, term: Term) -> Result {
        use Term::*;
    ...
    unsafe fn compile_closure(&mut self, env: Vec<(String, Term)>, term: Term) -> Result {
        self.enter_scope();
    ...
    unsafe fn compile_let(&mut self, entries: Vec<(String, Term)>, term: Term) -> Result {
        self.enter_scope();
    ...
    unsafe fn compile_if(&mut self, cond: Term, then: Term, otherwise: Term) -> Result {
        let current_fn = self.current_fn;
    ...
    unsafe fn compile_lam(&mut self, args: Vec<String>, term: Term) -> Result {
    ...
    unsafe fn compile_app(&mut self, callee: Term, args: Vec<Term>) -> Result {
        let current_fn = self.current_fn;
    ...
    unsafe fn make_closure_environment(&mut self, env: Vec<(String, Term)>) -> Result {
    ...
    unsafe fn make_closure(&mut self, env: Vec<(String, Term)>, body: Term) -> Result {
        ...
        let closure = self.make_call("prim__Value_new_closure", &mut [env_value, env_len, value]);
